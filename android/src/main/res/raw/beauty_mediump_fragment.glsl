// mediump build of RootEncoder's beauty_fragment.glsl, for budget GPUs that
// run highp at half rate. Texture COORDINATES stay highp so the 24-tap blur
// doesn't drift (mediump near 1.0 has ~1px error at 720p); the per-pixel
// COLOR math (pow, the hard-light loop, the saturation matrix) runs mediump —
// that's where the bandwidth/ALU saving is, and it's visually imperceptible.
