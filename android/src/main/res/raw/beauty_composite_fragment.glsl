// Beauty pass 5 of 5: three-band reconstruction at full resolution.
//
...
//
// Bright and Cool are stacked in one texture, four rows each, selected by
...
const float SKIN_FINE  = 0.30;   // suppress pores/noise, preserve hard edges below
const float SKIN_MED   = 1.20;   // restrained skin clarity
const float BG_MED     = 1.50;   // restrained off-skin clarity
const float SKIN_LIFT  = 0.05;
...
const float LUT_STACK = 8.0;   // rows in the whole atlas (LUT_ROWS * 2 looks)
...
void main() {
  ...
  // shader this replaces took 25 reads, so the whole guided pipeline is
  ...
  // Sensor noise raises `keep`, so using it here would disable smoothing on the
  // frames that need it most. `hard` opens later and preserves only real edges.
  float skinFine = mix(SKIN_FINE, 1.0, hard);
  ...
  color = mix(src, color, clamp(uBeautyIntensity, 0.0, 1.0));
